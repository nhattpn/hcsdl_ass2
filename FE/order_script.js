const API_URL = "http://localhost:5000/api/procedure/orderswithcustomerdetails"; // Thay đổi URL API 
...
const fetchOrders = async () => {
  const { customer_id, status } = getQueryParams();
...
const renderOrders = (orders) => {
  ...
  orders.forEach((order) => {
    const row = `
    <tr data-oid="${order.OID}" data-name="${order.FULL_NAME}" data-store-name="${order.SHOP_NAME}" data-status="${order.ORDER_STATUS}">
        ...
        <td class="py-3 px-6 border-b">${order.ORDER_STATUS}</td>
