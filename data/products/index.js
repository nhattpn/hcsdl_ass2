...
const createProduct = async (body)=> {
    try {
        ...
        let result = await pool.request()
            .input('image', type['image'], body.image[0].path)
            .query(`
                INSERT INTO Products (pid, image, manufacturer_date, current_price, name, description, avg_rating, remain_quantity, bid, cid, sid)
                VALUES ('${body.pid}', @image , '${body.manufacturer_date}', ${body.current_price}, '${body.name}', '${body.description}', ${body.avg_rating}, ${body.remain_quantity}, '${body.bid}', '${body.cid}', '${body.sid}')
            `);
        console.log(result)
        sql.close();
